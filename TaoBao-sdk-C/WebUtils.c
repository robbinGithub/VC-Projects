    ...
    if(webHander->curl) {
        ...
        if(startWith(url,"https")){
            if(capath != NULL){
                ...
                curl_easy_setopt(webHander->curl, CURLOPT_SSL_VERIFYPEER, 1);
            }else{
        ...
        if(params != NULL){
            for( i = 0 ; i != params->ite; i++){
                if(params->need_sign[i] == 1) {
                    if(curl_formadd(&post, &last,
                                    CURLFORM_COPYNAME, params->keys[i],
                                    CURLFORM_COPYCONTENTS, params->values[i], CURLFORM_END) != 0){
                        return CURLE_FAILED_INIT;
                    }
                }else{
                    if(curl_formadd(&post, &last, CURLFORM_COPYNAME, params->keys[i],
                                    CURLFORM_BUFFER,params->keys[i],
                                    CURLFORM_BUFFERPTR,params->values[i],
                                    CURLFORM_BUFFERLENGTH, params->value_lens[i],CURLFORM_END) != 0){
                        return CURLE_FAILED_INIT;
                    }
                }
            }
